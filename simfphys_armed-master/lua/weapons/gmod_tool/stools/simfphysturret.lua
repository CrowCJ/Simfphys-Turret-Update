...
if SERVER then
	...
	function TOOL:UpdateTurret( ent )
		...
		ent:SetShootDelay( math.Clamp(self:GetClientNumber( "delay" ),0.05,20) )
		ent:SetDamage( math.Clamp(self:GetClientNumber( "damage" ),0,8000) )
		ent:SetForce( math.Clamp(self:GetClientNumber( "force" ),0,15000) )
		ent:SetSize( math.Clamp(self:GetClientNumber( "size" ),0.5,25) )
		ent:SetDeflectAng( math.Clamp(self:GetClientNumber( "deflectang" ),0,45) )
		ent:SetBlastDamage( math.Clamp(self:GetClientNumber( "blastdamage" ),0,5000) )
...
function TOOL.BuildCPanel( CPanel )
	CPanel:AddControl( "ComboBox", { MenuButton = 1, Folder = "simfphys_turrets", 
